...
const normalizeMarkdown = (markdown: string): string => {
  ...
  output = output.replace(/\[([^\]]+)\]\(([^)\s]+(?:\s"[^"]+")?)\)/g, (match, text) => {
    return text;
  });
...
const fetchWikiMetadata = async (topic: Topic): Promise<ArticleMetadata> => {
    ...
    return {
        ...
        canonicalUrl: page.canonicalurl ?? page.fullurl ?? `https://en.wikipedia.org/wiki/${topic.wikipedia_slug}`,
...
const readLocalGrokCitations = (topic: Topic): ExternalCitation[] => {
    ...
    try {
        ...
        const payload = JSON.parse(raw) as {
            references?: Array<{ citation_id: string; raw?: string; normalized?: { title?: string | null; url?: string | null } }>;
        };
...
const fetchWiki = async (topic: Topic): Promise<void> => {
    const [wikitextPayload, metadata] = await Promise.all([fetchWikiWikitext(topic), fetchWikiMetadata(topic)]);
...
const runFetch = async (source: "wiki" | "grok", topicId?: string): Promise<void> => {
...
const fetchCommand = new Command("fetch")
    .description("Download topic content from Grokipedia and Wikipedia for offline comparison");
...
fetchCommand
    ...
    .action(async (options: { topic?: string }) => {
        await runFetch("wiki", options.topic);
    });

fetchCommand
    .command("grok")
    .description("Fetch articles from Grokipedia")
    .option("-t, --topic <id>", "Topic identifier (default: all topics)")
    .action(async (options: { topic?: string }) => {
        await runFetch("grok", options.topic);
    });
