...
import {CONFIG_PATH, readConfig, writeConfig} from "../shared/config";
...
const normalize = (value?: string | null) => (value && value.trim().length ? value.trim() : undefined);
...
const initCommand = new Command("init")
    ...
    .option("--epochs <number>", "Default epochs to retain published assets", (value) => Number(value))
    ...
    .action(async (options: InitOptions) => {
            ...
            [
                {
                    ...
                    validate: (value: string) => (value && value.trim().length ? true : "Endpoint is required.")
                },
                {
                ...
                },
                {
                    ...
                    validate: (value: string) => (value && value.trim().length ? true : "Blockchain is required.")
                },
                {
                    ...
                    validate: (value: string) => (value && value.trim().length ? true : "Private key is required.")
                },
                {
        ...
        const environment =
            normalize(options.environment) ?? normalize(responses.environment) ?? existing.dkgEnvironment ?? FALLBACK_ENVIRONMENT;
        ...
        const blockchain =
            normalize(options.blockchain) ?? normalize(responses.blockchain) ?? existing.dkgBlockchainId ?? FALLBACK_BLOCKCHAIN;
        const privateKey =
        ...
        const publicKey =
            ...
            FALLBACK_PUBLIC_KEY;
        const rpcUrl = normalizeUrl(options.rpc) ?? normalizeUrl(responses.rpcUrl) ?? existing.dkgRpcUrl;
        const publishEpochs = options.epochs ?? responses.publishEpochs ?? existing.publishEpochs ?? FALLBACK_EPOCHS;
        const publishMaxRetries =
            options.maxRetries ?? responses.publishMaxRetries ?? existing.publishMaxRetries ?? FALLBACK_MAX_RETRIES;
