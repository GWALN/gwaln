...
export const resolvePublishConfig = (
    ...
    if (!endpoint) {
        throw new Error("No DKG endpoint configured. Run 'civiclens init' or pass --endpoint explicitly.");
    }

    const environment =
        (overrides.environment ?? cfg.dkgEnvironment ?? FALLBACK_ENVIRONMENT).toLowerCase();
    const port = overrides.port ?? cfg.dkgNodePort ?? FALLBACK_PORT;
    const blockchain =
        normalizeBlockchain(overrides.blockchain ?? cfg.dkgBlockchainId) ??
        FALLBACK_BLOCKCHAIN;
    const privateKey =
        normalizeKey(overrides.privateKey ?? cfg.dkgPrivateKey) ??
        FALLBACK_PRIVATE_KEY;
    const publicKey =
        normalizeKey(overrides.publicKey ?? cfg.dkgPublicKey) ??
        FALLBACK_PUBLIC_KEY;
    const rpcUrl = cleanUrl(overrides.rpcUrl ?? cfg.dkgRpcUrl) ?? BLOCKCHAIN_RPC_FALLBACKS[blockchain];
    const epochsNum = overrides.epochsNum ?? cfg.publishEpochs ?? FALLBACK_EPOCHS;
    const maxRetries =
        overrides.maxRetries ??
        cfg.publishMaxRetries ??
        FALLBACK_PUBLISH_MAX_RETRIES;
    ...
    if (!privateKey) {
        throw new Error("No private key configured. Provide --private-key or update .civiclensrc.json.");
    }
