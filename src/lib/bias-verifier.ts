...
const buildPrompt = (events: DiscrepancyRecord[], wikiText: string, grokText: string, contextLimit: number): string => {
    const wikiContext = wikiText.length > contextLimit ? `${wikiText.slice(0, contextLimit)}…` : wikiText;
...
export const verifyBiasWithGemini = async ({
    ...
    try {
        ...
        throw new Error("Gemini response was not valid JSON. Enable debug logging to inspect the raw payload.");
    }
    return parsed
        ...
        .map((entry) => ({
            ...
            confidence: typeof entry.confidence === "number" ? Number(clamp(entry.confidence).toFixed(2)) : null,
