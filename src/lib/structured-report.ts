...
import type {NumericDiscrepancy, EntityDiscrepancy} from "./discrepancies";
import type {SectionAlignmentRecord, ClaimAlignmentRecord} from "./alignment";
...
const buildHeadline = (topic: Topic, summary: StructuredAnalysisSummary): string => {
    ...
    if (summary.hallucination_count) {
        parts.push(formatCount(summary.hallucination_count, "hallucination flag", "hallucination flags"));
    }
    if (!parts.length) {
...
export const buildStructuredAnalysis = (topic: Topic, payload: AnalysisPayload): StructuredAnalysisReport => {
