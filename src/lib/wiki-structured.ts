...
const getTemplateBlock = (text: string, startIndex: number): { block: string; end: number } | null => {
    let depth = 0;
...
const stripTables = (text: string): string => {
...
const stripWikiMediaMarkup = (
    ...
    while ((match = regex.exec(text)) !== null) {
        ...
        const params = paramsRaw.split("|").map((value) => value.trim()).filter(Boolean);
...
const stripWikiCitations = (text: string, references: ReferenceStore): { text: string; matches: ReferenceMatch[] } => {
    let output = "";
...
const parseParagraph = (
    ...
    const mediaStripped =
        mode === "wiki" ? stripWikiMediaMarkup(trimmed, media, sectionId) : stripMarkdownMedia(trimmed, media, sectionId);
...
const buildWikiSections = (
    ...
    for (let i = 0; i < matches.length; i += 1) {
        ...
        const mediaIds = Array.from(
            new Set(paragraphs.flatMap((paragraph) => paragraph.sentences.flatMap((sentence) => sentence.media_ids)))
        );
        sections.push({
...
const buildMarkdownSections = (
    ...
    for (let i = 0; i < matches.length; i += 1) {
        ...
        const blocks = content.split(/\n\s*\n/);
        blocks.forEach((block, idx) => {
            const paragraph = parseParagraph(block, sectionId, idx, references, media, sectionId, "markdown");
            if (paragraph) {
                paragraphs.push(paragraph);
            }
        });
    ...
    return Array.from(
        new Set(
            matches
                ...
                .filter((value) => value.split(" ").length <= 4 && value.length > 2 && !/^[A-Z][a-z]?$/.test(value))
        )
    );
};

const extractEntities = (text: string): Array<{ label: string; type: string | null; qid: string | null }> => {
...
const extractNumbers = (text: string): Array<{ raw: string; value: number; unit: string | null }> => {
...
class MediaRegistry {
    ...
    registerBodyMedia(input: RegisterBodyMediaInput): string {
        ...
        if (!this.media.has(mediaId)) {
            ...
            existing?.usage.push({context: input.context, section_id: input.sectionId, sentence_id: null});
        }
        return mediaId;
    }
...
class ReferenceStore {
    ...
    private createReference(name: string | null, raw: string, inner: string | null): StructuredReference {
    ...
    toArray(): StructuredReference[] {
        return [...this.byName.values(), ...this.anon.values(), ...this.links.values(), ...this.external.values()];
    }
}
...
const normalizeReference = (
    ...
    if (!inner) {
        return {type: null, title: null, publisher: null, journal: null, year: null, url: null, doi: null};
    }
    const citeMatch = inner.match(/\{\{\s*cite\s+([^\s|}]+)([^}]*)\}\}/i);
    if (!citeMatch) {
        return {type: null, title: cleanSentenceText(inner) || null, publisher: null, journal: null, year: null, url: null, doi: null};
    }
    const type = citeMatch[1]?.trim().toLowerCase() ?? null;
    const fieldsText = citeMatch[2] ?? "";
    const fieldParts = fieldsText.split("|").map((part) => part.trim()).filter(Boolean);
...
export const parseWikiArticle = (
    ...
    const leadEnd = firstHeadingMatch && firstHeadingMatch.index !== undefined ? firstHeadingMatch.index : cleaned.length;
...
export const parseMarkdownStructuredArticle = (
    ...
    const sections = normalizedBodyText ? buildMarkdownSections(normalizedBodyText, referenceStore, mediaRegistry) : [];
    ...
    if (!lead.paragraphs.length && sections.length) {
        ...
        if (candidateParagraph) {
            ...
            lead = {
                text_range: {start_offset: 0, end_offset: clonedSentences.reduce((sum, sentence) => sum + sentence.text.length, 0)},
