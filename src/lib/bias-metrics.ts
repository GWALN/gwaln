...
const positiveWords = ["reliable", "credible", "scientific", "robust", "well-established", "trusted"];
...
const scoreSubjectivity = (tokens: string[]): number => {
    const subjective = tokens.filter((token) => positiveWords.includes(token) || negativeWords.includes(token));
