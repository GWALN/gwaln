...
const snip = (value: string, limit = 240): string => (value.length > limit ? `${value.slice(0, limit)}…` : value);
...
export const buildCommunityNote = (
    ...
    return {
        ...
        "hasPart": discrepancies.map((issue, idx) => annotationFromDiscrepancy(issue, topic.id, urls, idx)),
        "citation": [
