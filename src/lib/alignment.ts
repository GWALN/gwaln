...
export const alignSections = (wiki: StructuredArticle, grok: StructuredArticle): SectionAlignmentRecord[] => {
    ...
    for (const section of wiki.sections) {
        ...
        for (const candidate of grok.sections) {
            ...
            const similarity = stringSimilarity.compareTwoStrings(normalizeHeading(heading), normalizeHeading(candidate.heading));
...
export const alignClaims = (wiki: StructuredArticle, grok: StructuredArticle): ClaimAlignmentRecord[] => {
