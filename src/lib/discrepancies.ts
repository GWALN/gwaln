...
export const detectNumericDiscrepancies = (alignments: ClaimAlignmentRecord[]): NumericDiscrepancy[] => {
...
export const detectEntityDiscrepancies = (alignments: ClaimAlignmentRecord[]): EntityDiscrepancy[] => {
    const discrepancies: EntityDiscrepancy[] = [];
    alignments.forEach((record) => {
        if (!record.wikipedia || !record.grokipedia) return;
        const wikiEntities = new Set(
            (record.wikipedia.entities ?? []).map((entity) => normalizeEntity(entity)).filter((label) => label.length > 0)
        );
        const grokEntities = new Set(
            (record.grokipedia.entities ?? []).map((entity) => normalizeEntity(entity)).filter((label) => label.length > 0)
        );
