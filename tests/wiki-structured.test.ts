...
import {ArticleMetadata, parseMarkdownStructuredArticle, parseWikiArticle} from "../src/lib/wiki-structured";
...
describe("parseWikiArticle", () => {
    it("parses lead sentences from the full Moon article", () => {
        ...
        expect(article.lead.text_range.end_offset).toBeGreaterThan(article.lead.text_range.start_offset);
...
Multiple probes have visited Venus.`;
        ...
        expect(article.sections.map((section) => section.heading)).toEqual(["Atmosphere", "Exploration"]);
