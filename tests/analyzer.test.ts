...
const toStructured = (markdown: string, source: "wikipedia" | "grokipedia" = "wikipedia"): StructuredArticle => {
...
describe("analyzeContent", () => {
    ...
    it("caps missing/extra sentences at five entries", () => {
        const wikiSentences = Array.from({length: 6}).map((_, idx) => paragraph(`Sentence ${idx + 1}: baseline.`));
