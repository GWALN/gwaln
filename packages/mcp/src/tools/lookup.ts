...
export const lookupHandler = async (input: z.infer<typeof LookupInputSchema>) => {
  ...
  if (action === 'add') {
    if (!input.title || !input.wikipediaSlug || !input.grokipediaSlug) {
      throw new Error('Missing required fields for adding topic: title, wikipediaSlug, grokipediaSlug');
    }
    ...
    return {
      content: textContent(`[lookup] Added topic "${newTopic.title}" to catalog (ID: ${newTopic.id})`),
      structuredContent: {
        action: 'add',
        topic: newTopic,
      },
  ...
  if (result.found && result.topic) {
    return {
      content: textContent(`[lookup] Found topic "${result.topic.title}" in local catalog (ID: ${result.topic.id})`),
      structuredContent: {
        action: 'search',
        found: true,
        topic: result.topic,
