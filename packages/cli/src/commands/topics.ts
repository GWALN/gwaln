...
import { paths, Topic, writeTopics } from '@gwaln/core';
...
const extractSlugFromLinks = (
  record: TopicFeedRecord,
  field: 'wikipedia' | 'grokipedia',
): string | undefined => {
  const directSlug =
    field === 'wikipedia'
      ? (record.wikipedia_slug ?? record.wikipediaUrl?.split('/').pop())
      : record.grokipedia_slug;
  if (directSlug) {
  ...
  const link =
    record.links?.[field] ??
    (field === 'wikipedia'
      ? (record.wikipedia_url ?? record.wikipediaUrl)
      : (record.grokipedia_url ?? record.grokipediaUrl));
  ...
  return {
    id:
      typeof record.id === 'string' && record.id.length
        ? record.id
        : (wikipediaSlug.toLowerCase() ?? `topic-${fallbackIndex}`),
    title,
    ...
    category:
      ...
      (typeof record.metadata?.['category'] === 'string'
        ? (record.metadata?.['category'] as string)
        : undefined),
  };
};
...
const useBundledCatalog = (destination: string): void => {
  ...
  console.log(`[topics] Using bundled topic catalog (${bundled.length} topics) at ${destination}.`);
};
...
topicsCommand
  ...
  .action(async (options: { source?: string; output?: string }) => {
    ...
    try {
      ...
      console.warn(
        `[topics] Failed to load source (${(error as Error).message}). Using bundled catalog.`,
      );
      useBundledCatalog(outputPath);
    }
  });
